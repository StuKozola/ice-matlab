...
9. **SFTP transport with atomic downloads.** `FtpSession` wraps MATLAB's built-in `sftp` (R2023a+); host rotation across `eod11/eod12/eod13.icedataservices.com` on connect failure. Each file downloads to `<name>.part` and is renamed on success so a killed transfer never leaves a corrupt cached file — important for the FTPCSD files that can hit 260 MB. Password auth initially; SSH key support is a hook for later.
...
3. **FTP transport** — SFTP via MATLAB's built-in `sftp` object (R2023a+). Password auth initially; SSH key support is a hook for later. Atomic `.part` → rename downloads for large FTPCSD files.
